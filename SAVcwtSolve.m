...
if spOnlyBool==0
    ...
    coSoln(1:coND1,:) = invertSol(coCompConvTrim,coNDec,1,coND1,respThresh,coNFiltD1,coOmega{1},deltaT);
    coSoln(coND1+1:coND1+coND2,:) = invertSol(coCompConvTrim,coNDec,coND1+1,coND1+coND2,respThresh,coNFiltD2,coOmega{2},deltaT);
    coSoln(coND1+coND2+1:coND1+coND2+coND3,:) = invertSol(coCompConvTrim,coNDec,coND1+coND2+1,coND1+coND2+coND3,respThresh,coNFiltD3,coOmega{3},deltaT);
    coSoln(coND1+coND2+coND3+1:coND1+coND2+coND3+coND4,:) = invertSol(coCompConvTrim,coNDec,coND1+coND2+coND3+1,coND1+coND2+coND3+coND4,respThresh,coNFiltD4,coOmega{4},deltaT);

    [coSolnWts,coSolnAmps,coSolnPhases,coTimesAll,coSolnFunc] = solnProcess(coTimes,coSoln,coN,coNDec,coRadians,coRadiansOrig,tPts,coTimeRel,rad2deg,ampFloor,ampLimit,coMaxFiltLength,decFact,nData,timeIn,coOmegaFlat);
...
spSoln = invertSol(spCompConvTrim,spNDec,1,spN,respThresh,spNFilt,spOmega,deltaT);

[spSolnWts,spSolnAmps,spSolnPhases,spTimesAll,spSolnFunc] = solnProcess(spTimes,spSoln,spN,spNDec,spRadians,spRadiansOrig,tPts,spTimeRel,rad2deg,ampFloor,ampLimit,spMaxFiltLength,decFact,nData,timeIn,spOmega);
...
%% monthly etc. filters
moSoln = invertSol(moCompConvTrim,moNDec,1,moN,respThresh,moNFilt,moOmega,deltaT);

[moSolnWts,moSolnAmps,moSolnPhases,moTimesAll,moSolnFunc] = solnProcess(moTimes,moSoln,moN,moNDec,moRadians,moRadiansOrig,tPts,moTimeRel,rad2deg,ampFloor,ampLimit,moMaxFiltLength,decFact,nData,timeIn,moOmega);

%% and possibly dynamic inference solution
if dynInfFlag==1 || dynInfCustFlag==1
    
    diSoln = invertSol(diCompConvTrim,diNDec,1,diN,respThresh,diNFilt,diOmega,deltaT);

    [diSolnWts,diSolnAmps,diSolnPhases,diTimesAll,diSolnFunc] = solnProcess(diTimes,diSoln,diN,diNDec,diRadians,diRadiansOrig,tPts,diTimeRel,rad2deg,ampFloor,ampLimit,diMaxFiltLength,decFact,nData,timeIn,diOmega);
...
function solnAll = invertSol(filtOutput,nVals,indStart,indStop,respThresh,cnf,cod,dt)
...
function [solnWts,solnAmpsAll,solnPhasesUnshiftedAll,timesAll,solnFunc] = solnProcess(timesIn,soln,N,nDec,radians,radiansOrig,tPts,timeRel,rad2deg,ampFloor,ampLimit,maxFL,decFact,nData,timeInUnshifted,omega)
...
%         (solnWts{it2,it3} < 0.01) * (0),1:N,1:nDec));           % phases relative to non-centered time axis; angle(exp(1i * timeRel(it2,it3)))
    
    solnPhasesUnshifted = cell2mat(cellit(@(it2,it3)(solnWts{it2,it3} > 0.01) * mod(2*pi*timesIn(it3)*(omega(it2)/3600/(2*pi)) - angle(soln(it2,it3)),2*pi) * rad2deg +...
        (solnWts{it2,it3} < 0.01) * (0),1:N,1:nDec));
